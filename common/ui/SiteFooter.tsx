...
const productLinks = [
  ...
  { href: "/agent", en: "SlideX Agent", zh: "SlideX Agent" },
  { href: "/docs", en: "Docs", zh: "文件" },
  { href: "/blog", en: "Blog", zh: "部落格" }
];

const resourceLinks = [
  { href: "https://heddleagent.com", en: "Heddle", zh: "Heddle" },
...
export function SiteFooter({ background }: { background?: ReactNode }) {
  const { locale, localePath } = useI18n();
  const isZh = locale === "zh-TW";

  return (
    <footer className="relative isolate overflow-hidden border-t border-white/10 bg-[#11130f] px-5 pb-7 pt-14 sm:px-7 lg:px-10 lg:pt-16">
      {background}
      <div className="relative z-10 mx-auto max-w-[1200px]">
        <div className="grid gap-12 md:grid-cols-[minmax(0,1.4fr)_minmax(0,0.5fr)_minmax(0,0.5fr)_minmax(0,0.5fr)] md:gap-8">
          <div>
            ...
            <p className="mt-5 max-w-xs text-[15px] leading-7 text-white/48">
          ...
          <div>
            ...
            <nav aria-label={isZh ? "產品連結" : "Product links"} className="mt-4 grid gap-3 text-[15px]">
          ...
          <div>
            ...
            <nav aria-label={isZh ? "資源連結" : "Resource links"} className="mt-4 grid gap-3 text-[15px]">
          ...
          <div>
            ...
            <nav aria-label={isZh ? "法律資訊連結" : "Legal links"} className="mt-4 grid gap-3 text-[15px]">
              {legalLinks.map((link) => (
                <Link className={`w-fit rounded-sm text-white/49 transition-colors hover:text-white ${focusRing}`} href={localePath(link.href)} key={link.href}>
                  {isZh ? link.zh : link.en}
                </Link>
        ...
        <div className="mt-14 flex flex-col gap-3 border-t border-white/10 pt-5 text-[12px] text-white/38 sm:flex-row sm:items-center sm:justify-between">
