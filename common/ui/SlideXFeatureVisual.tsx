...
export function SlideXFeatureVisual({ variant }: { variant: SlideXFeatureVisualVariant }) {
  ...
  return (
    <svg aria-label={ariaLabel} className="h-full w-full" role="img" viewBox="0 0 800 500">
      ...
      <motion.g animate={reduceMotion ? undefined : { opacity: [.82, 1, .82] }} transition={transition}>
      ...
      <motion.g animate={reduceMotion ? undefined : { x: variant === "shader" ? [0, 68, 0] : [68, 0, 68] }} transition={transition}>
        ...
        <g key={item}>
          ...
          <motion.g
            animate={reduceMotion ? undefined : {
              scaleX: [(36 + item * 8) / 50, (88 - item * 4) / 50, (36 + item * 8) / 50]
            }}
            style={{ transformBox: "fill-box", transformOrigin: "left center" }}
            transition={{ ...transition, delay: item * .2 }}
          >
            <rect fill={item === 1 ? "#c4ee87" : "#9ad7ff"} height="4" rx="2" width="50" x="658" y={189 + item * 54} />
          </motion.g>
        </g>
      ))}
    </svg>
